...
import {
   ...
   createCalendarEventSchemaArgs,
   addCalendarEventResponseSchema,
} from '@/schemas/calendar/block-calendar-event-schema'

export const createCalendarEvent = async (
   url: string,
   { arg }: Readonly<{ arg: CreateCalendarEventArgs }>,
) => {
   const argsValidationResult = createCalendarEventSchemaArgs.safeParse(arg)
   ...
   try {
      ...
      const responseValidationResult = addCalendarEventResponseSchema.safeParse(data)
