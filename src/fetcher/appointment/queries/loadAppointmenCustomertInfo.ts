...
import {
   appointmentCustomerInfoSchema,
   LoadAppointmentCustomerResponse,
} from '@/schemas/appointment/appointment-customer-info-schema'

export const loadAppointmentCustomerInfo: Fetcher<
   LoadAppointmentCustomerResponse,
...
> = async ({ url, id }) => {
   try {
      ...
      const responseValidationResult = appointmentCustomerInfoSchema.safeParse(data)
