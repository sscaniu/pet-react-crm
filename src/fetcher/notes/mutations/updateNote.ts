...
import { UpdateNoteArgs, updateNoteSchema } from '@/schemas/notes/notes-schema'

export const updateNote = async (url: string, { arg }: Readonly<{ arg: UpdateNoteArgs }>) => {
   const argsValidationResult = updateNoteSchema.safeParse(arg)
   ...
   try {
      ...
      const responseValidationResult = updateNoteSchema.safeParse(data)
