...
import { listAppointmentHistoryByPetId } from '@/fetcher/appointment/queries/listAppointmentsHistoryByPetId'
...
export default function AppointmentHistoryTable() {
   ...
   const {
      ...
      petInformationData?.id
         ? ['/appointmentV3/loadPetAppointmentHistory', petInformationData?.id]
         : null,
      ([url, petId]) => listAppointmentHistoryByPetId({ url, petId }),
   )
   ...
   return (
      <>
         {listAppointmentHistoryData?.length ? (
            ...
            <EmptyPlaceholder message="No appointment history found for this pet." />
         )}
      </>
   )
}
